...
if(isset($_POST["submit"])){
    ...
    if($type=="Privée"){
        $sqlInsert="insert into `université_etablissment_prive` (NomUni,VilleUni,AdresseUni,NomEtab,SigleEtab,type,image,liensite) 
        values('$nomU','$vile','$adr','$nomE','$sigEtab','$type','$img','$lien')";

    }

    if($type=="Public"){
        $sqlInsert="insert into `université_etablissment_public` (NomUni,VilleUni,AdresseUni,NomEtab,SigleEtab,type,image,liensite) 
        values('$nomU','$vile','$adr','$nomE','$sigEtab','$type','$img','$lien')";

    }

    $res=$con->exec($sqlInsert);

    if($res){
        header("location:ecoles1.php");
    }
    else{
        return alert("Error d'insertion !");
    }
